...
for i, row in input_df.iterrows():
    ...
    targets = []
    targets = [gvef.getVertexId(vertex_df, col_name + '_' + str(row[col_name])) for col_name in columns_names[1:] if
               col_name != 'title']

    for name_attr, df in dict_ma_df.items():
        foundValue = df.loc[df['movieId'] == row['movieId'], name_attr].tolist()
        targets = [gvef.getVertexId(vertex_df, name_attr + '_' + str(fv)) for fv in foundValue]
